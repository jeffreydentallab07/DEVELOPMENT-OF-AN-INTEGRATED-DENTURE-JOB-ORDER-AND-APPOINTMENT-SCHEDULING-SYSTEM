...
<div class="p-4 md:p-6 bg-gray-300 min-h-screen">
    <div class="max-w-full mx-auto">
        ...
        <div class="mb-4 md:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Case Orders</h1>
            <a href="{{ route('clinic.case-orders.create') }}"
                class="w-full sm:w-auto text-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition">
                + New Case Order
            </a>
        </div>
        ...
        <!-- Success Message -->
        @if(session('success'))
        <div class="mb-4 md:mb-6 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded">
            {{ session('success') }}
        </div>
        @endif
        ...
        <!-- Error Message -->
        @if(session('error'))
        <div class="mb-4 md:mb-6 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded">
            {{ session('error') }}
        </div>
        @endif

        <!-- Search and Filter Section -->
        <div class="bg-white shadow-sm rounded-lg mb-4 md:mb-6">
            <div class="p-4 md:p-6">
                <form method="GET" action="{{ route('clinic.case-orders.index') }}" class="space-y-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('clinic.case-orders.index') }}"
                            class="flex-1 text-center bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg transition">
                            Reset
                        </a>
                    </div>
        ...
        @if($caseOrders->total() > 0)
        <div class="hidden md:flex justify-between items-center text-sm text-gray-600 mb-4">
            <div>
                Showing {{ $caseOrders->firstItem() }} to {{ $caseOrders->lastItem() }} of {{ $caseOrders->total() }}
                results
            </div>
            <div>
        ...
        <div class="hidden md:block bg-white shadow-sm rounded-lg overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-blue-900">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Case ID
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Patient
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Dentist
                        </th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">
                            Technician</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Created
                        </th>
        ...
        <div class="md:hidden space-y-4">
            ...
            <div class="bg-white rounded-lg shadow-sm p-4">
                ...
                <div class="flex justify-between items-start mb-3">
                    ...
                    @endphp
                    <span
                        class="px-2 py-1 rounded-full text-xs font-semibold {{ $statusColors[$caseOrder->status] ?? 'bg-gray-100 text-gray-800' }}">
                        {{ ucfirst($caseOrder->status) }}
                    </span>
                </div>
                ...
                <div class="space-y-2 text-sm mb-4">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-900">{{ $caseOrder->patient->name }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Dentist:</span>
                        <span class="font-medium text-gray-900">{{ $caseOrder->dentist->name }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Type:</span>
                        <span class="font-medium text-gray-900">{{ ucfirst($caseOrder->case_type) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Technician:</span>
                        <span class="font-medium text-gray-900">{{ $caseOrder->latestAppointment?->technician?->name ??
                            '-' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Created:</span>
                        <span class="font-medium text-gray-900">{{ $caseOrder->created_at->format('M j, Y') }}</span>
                    </div>
                </div>
                ...
                <div class="flex gap-2 pt-3 border-t border-gray-200">
                    ...
                    @if($caseOrder->status === 'under review')
                    <a href="{{ route('clinic.case-orders.review', $caseOrder->co_id) }}"
                        class="flex-1 text-center bg-purple-600 text-white py-2 px-3 rounded text-sm font-medium hover:bg-purple-700 transition">
                        Review
                    </a>
                    @endif
                    @if(in_array($caseOrder->status, ['pending', 'adjustment requested']))
                    <a href="{{ route('clinic.case-orders.edit', $caseOrder->co_id) }}"
                        class="flex-1 text-center bg-green-600 text-white py-2 px-3 rounded text-sm font-medium hover:bg-green-700 transition">
                        Edit
                    </a>
                    @endif
                    @if($caseOrder->status === 'pending')
                    <form action="{{ route('clinic.case-orders.destroy', $caseOrder->co_id) }}" method="POST"
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="bg-red-600 text-white py-2 px-3 rounded text-sm font-medium hover:bg-red-700 transition">
                            Delete
                        </button>
                    </form>
                    @endif
